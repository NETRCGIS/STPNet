...
from core.models import predrnn, predrnn_v2, shuffle_sa_stp
...
class Model(object):
    def __init__(self, configs):
        ...
        networks_map = {
            ...
            # 'sa_predrnn_v2': sa_predrnn_v2_2.RNN,
            'shuffle_sa_predrnn_v2': shuffle_sa_stp.RNN,
        }

        if configs.model_name in networks_map:
            Network = networks_map[configs.model_name]
            self.network = Network(
                self.num_layers, self.num_hidden, configs).to(configs.device)
        else:
