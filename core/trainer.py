...
def test(model, test_input_handle, configs, itr):
    ...
    for i in range(configs.total_length - configs.input_length):
        ...
        # MSE OF IMAGES
        sum_imgmse += img_mse[i] / (batch_id * configs.batch_size)
    print("avg_imgmse："+str(sum_imgmse/(configs.total_length - configs.input_length)))
    avg_imgmse = np.asarray(img_mse, dtype=np.float32) / \
        (configs.batch_size * batch_id)
    print('MSE per frame: ' + str(np.mean(avg_imgmse)))
    ...
    #     for i in range(output_length):
    #         anstxt.write(
    #             str(img_mse[i] / (batch_id * configs.batch_size * configs.n_gpu)) + '\n')
    #     anstxt.write('rainfall_mae per seq: ' + str(avg_rmse) + '\n')
    #     for i in range(output_length):
    #         anstxt.write(
    #             str(img_rmse[i] / (batch_id * configs.batch_size * configs.n_gpu)) + '\n')
